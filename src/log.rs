		...
		match buf[0] {
			...
			4 => {  // EndRecord
				...
				if self.validate {
					...
					log::trace!(target: "axia-db", "Read end of record");
				}
...
impl LogChange {
		...
		-> Result<(HashMap<IndexTableId, IndexLogOverlay>, HashMap<ValueTableId, ValueLogOverlay>, u64)>
	{
		...
		for id in self.dropped_tables.iter() {
			log::debug!(target: "axia-db", "Finalizing drop {}", id);
...
impl Log {
				...
				if entry.metadata()?.is_file() && name.starts_with("log") {
					if let Ok(nlog) = std::str::FromStr::from_str(&name[3..]) {
						...
						if let Some(record_id) = record_id {
							log::debug!(target: "axia-db", "Opened log {}, record {}", nlog, record_id);
							logs.push_back((nlog, record_id, file));
							if nlog > max_log_id {
								max_log_id = nlog
							}
						} else {
							log::debug!(target: "axia-db", "Removing log {}", nlog);
							std::mem::drop(file);
	...
	pub fn open_log_file(path: &std::path::Path) -> Result<(std::fs::File, Option<u64>)> {
		...
		log::debug!(target: "axia-db", "Opened existing log {}, first record_id = {}", path.display(), id);
		Ok((file, Some(id)))
	}

	fn drop_log(&self, id: u32) -> Result<()> {
		log::debug!(target: "axia-db", "Drop log {}", id);
	...
	pub fn flush_one(&self, min_size: u64) -> Result<(bool, bool, bool)> {
		...
		if flushing.is_some() {
			...
			while *reading_state == ReadingState::Reading  {
				log::debug!(target: "axia-db", "Flush: Awaiting log reader");
...

			{
				...
				if let Some(reading) = reading.take() {
					log::debug!(target: "axia-db", "Flush: Activated log cleanup {}", reading.id);
				...
				if let Some(mut flushing) = flushing.take() {
					log::debug!(target: "axia-db", "Flush: Activated log reader {}", flushing.id);
		...
		if self.sync {
			if let Some(flushing) = flushing.as_ref() {
				log::debug!(target: "axia-db", "Flush: Flushing log to disk");
				flushing.file.sync_data()?;
				log::debug!(target: "axia-db", "Flush: Flushing log completed");
			}
		}
	...
	pub fn replay_next(&mut self) -> Result<Option<u32>> {
		...
		{
			if let Some(reading) = reading.take() {
				log::debug!(target: "axia-db", "Replay: Activated log cleanup {}", reading.id);
				let file = reading.file.into_inner();
				self.cleanup_queue.write().push_back((reading.id, file));
			}
		}
		if let Some((id, _record_id, file)) = self.replay_queue.write().pop_front() {
			log::debug!(target: "axia-db", "Replay: Activated log reader {}", id);
			*reading = Some(Reading {
				id,
	...
	pub fn clean_logs(&self, count: usize) -> Result<bool> {
		...
		for (id, ref mut file) in cleaned.iter_mut() {
			log::debug!(target: "axia-db", "Cleaned: {}", id);
			file.seek(std::io::SeekFrom::Start(0))?;
			file.set_len(0)?;
		}
	...
	pub fn read_next<'a>(&'a self, validate: bool) -> Result<Option<LogReader<'a>>> {
		...
		if *reading_state != ReadingState::Reading {
			log::trace!(target: "axia-db", "No logs to enact");
			return Ok(None);
		}

		let reading = self.reading.write();
		if reading.is_none() {
			log::trace!(target: "axia-db", "No active reader");
			return Ok(None);
		}
		...
		match reader.next() {
			...
			Err(Error::Io(e)) if e.kind() == std::io::ErrorKind::UnexpectedEof => {
				*reading_state = ReadingState::Idle;
				...
				log::debug!(target: "axia-db", "Read: End of log");
				return Ok(None);
			}
			Err(e) => return Err(e),
		};
